...
class Obstacles:
    ...
    def show_CDF(self, ax, show_CA_as_size):
        ...
        ax.hist(np.array(self.CA_lengths) * F, num_bins, density=True, histtype='step', cumulative=True, label="Simulated {:1.0f} trials".format(self.trials_count), edgecolor='black', linewidth=1.5)
                ...
                for index_w, w in enumerate(width):
                    for index_l, l in enumerate(length):
                        if False:
                            ...
                            minkowski_area = self.Minkowski_sum_convex_polygons_area(self.CA_width, x_val, w, l, orientation_val, 0)
