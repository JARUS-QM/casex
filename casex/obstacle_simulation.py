...
class CObstacleSimulation:
    ...
    def generate_rectangular_obstacles_normal_distributed(self, num_of_obstacles, width_mu, width_sigma, length_mu, length_sigma):
        """
        """
    ...
    def generate_CAs(self, trials_count, CA_width, CA_length):
        ...
        # Compute reduction in the translation of the original CA. Since rotation is around the center, the compensation is half the longest length of the CA
        ...
        CA_trans_x = sp.stats.uniform.rvs(size = self.trials_count, loc = CA_compensate, scale = self.trial_area_sidelength - 2 * CA_compensate)
        CA_trans_y = sp.stats.uniform.rvs(size = self.trials_count, loc = 0, scale = self.trial_area_sidelength - 2 * CA_compensate)
        
        CA_coor = [(0, 0), (self.CA_width, 0), (self.CA_width, self.CA_length), (0, self.CA_length), (0, 0)]

        for j in range(0, self.trials_count):
            # Rotate and move CA
            CA_polygon = affinity.translate(affinity.rotate(Polygon(CA_coor), heading[j], 'center'), CA_trans_x[j], CA_trans_y[j])
        ...
        for CA_idx, CA_polygon in enumerate(self.CAs):        
            ...
            for idx, obstacle in enumerate(potentially_intersecting_obstacles):
                ...
                if CA_polygon.intersects(obstacle):
                    ...
                    CA_beginning_line = LineString([(CA_original_coords[0].x, CA_original_coords[0].y), (CA_original_coords[1].x, CA_original_coords[1].y)])
                    if CA_beginning_line.intersects(obstacle):
                        ...
                        if CA_splitted.geom_type == 'MultiPolygon':
                            ...
                            for CA_split in CA_splitted:
                                ...
                                if (CA_split.distance(CA_original_coords[0]) < self.__epsilon) | (CA_split.distance(CA_original_coords[1]) < self.__epsilon):
                                    CA_polygon = CA_split
                                    break
    ...
    def __cut_polygon_to_rectangle(self, CA_polygon, CA_original_coords):
        ...
        This function finds all the coordinates of the non-rectangular (splitted) CA that were not in the original CA
        ...
        dist = np.abs(y*p_closest.x - x*p_closest.y + CA_original_coords[1].x*CA_original_coords[0].y - CA_original_coords[1].y*CA_original_coords[0].x) / d
        ...
        CA_polygon = Polygon([[CA_original_coords[0].x, CA_original_coords[0].y], [CA_original_coords[1].x, CA_original_coords[1].y], [point2.x, point2.y], [point1.x, point1.y]])
    ...
    def show_simulation(self, ax, **options):
        ...
        if options.get("obstacles_original") == "True":
            for o in self.obstacles:
                ax.add_patch(PolygonPatch(o, facecolor='#00ff00', edgecolor='#000000', alpha=1, zorder=2, linewidth=0.25))
    
        if options.get("obstacles_intersected") == "True":
            for o in self.intersected_obstacles:
                ax.add_patch(PolygonPatch(o, facecolor='#ff8800', edgecolor='#000000', alpha=1, zorder=2, linewidth=0.25))
    ...
    def show_CDF(self, ax, show_CA_as_size):
        ...
        ax.hist(np.array(self.CA_lengths) * F, bins, density=True, histtype='step', cumulative=True, label="Simulated {:1.0f} trials".format(self.trials_count), edgecolor='black', linewidth=1.5)
    ...
    def set_limits(self, ax, x0, xN, y0, yN, step = 1):
        ax.set_xlim(x0, xN)
    ...
    def Minkowski_sum_convex_polygons(self, A, B):
        """
    ...
    def Minkowski_sum_convex_polygons_area(self, w, x, a, b, theta, theta2):
        """
    ...
    def Minkowski_difference_convex_polygons(self, A, B):
        """
    ...
    def mirror_polygon_in_origin(self, polygon):
        """
        """
    ...
    def first_pdf(self, x, obstacle_density, obstacle_width_mu, obstacle_width_sigma, obstacle_length_mu, obstacle_length_sigma, pdf_resolution):
        """
        """
        
        # Sample the obstacle PDF
        width = np.linspace(obstacle_width_mu - 3*obstacle_width_sigma, obstacle_width_mu + 3*obstacle_width_sigma, pdf_resolution)
        length = np.linspace(obstacle_length_mu - 3*obstacle_length_sigma, obstacle_length_mu + 3*obstacle_length_sigma, pdf_resolution)
                ...
                for index_w, w in enumerate(width):
                    for index_l, l in enumerate(length):
                        if False:
                            ...
                            area = self.Minkowski_sum_convex_polygons_area(self.CA_width, x_val, w, l, orientation_val, 0)
