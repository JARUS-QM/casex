...
def example14_obstacle_simulation():
    ...
    if do_theory:
        ...
        pdf_resolution = 20
        p_x, sanity_check = OS.first_pdf(x, obstacle_density, obstacle_width_mu, obstacle_width_sigma, obstacle_length_mu, obstacle_length_sigma, pdf_resolution)
    ...
    if do_viz:
        ...
        OS.show_simulation(ax1, CAs = "True", CAs_reduced = "True", obstacles_original = "True", obstacles_intersected = "True", CA_first_point = "False", debug_points = "False")
    ...
    if do_sanity_check & do_viz:
        ...
        for CAr in problematic_CAs:
            ax1.add_patch(PolygonPatch(CAr, facecolor='#ffff00', edgecolor='#000000', alpha=1, zorder=10, linewidth=0.25))
    ...
    print('Reduced CA:              {:1.0f} m^2 ({:d}%)'.format(np.mean(OS.CA_lengths) * OS.CA_width, int(round(100 * np.mean(OS.CA_lengths) / OS.CA_length))))

    print('---------------------------')
    if do_sanity_check:
        print('Total intersection area: {:1.0f} m^2'.format(problematic_area))
    print('Number of CA reduced:    {:d}   ({:d}%)'.format(OS.num_of_reduced_CA, int(round(100*OS.num_of_reduced_CA/OS.trials_count))))
    print('Number of CA empty:      {:d}   ({:d}%)'.format(OS.num_of_empty_CA, int(round(100*OS.num_of_empty_CA/OS.trials_count))))
