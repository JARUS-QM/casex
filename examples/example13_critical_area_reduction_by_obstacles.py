...
def example13_critical_area_reduction_by_obstacles():
    ...
    for k in range(2):
        for j in range(2):
            y, no_obstacles_in_CA = CA.simulate_minimum_CDF(obstacle_density[c], width, distance_glide_slide,
                                                            trials_count)
            y2, x, p_none, Ac_reduced = CA.compute_minimum_CDF(obstacle_density[c], width, distance_glide_slide,
                                                               resolution, 0.8)
            ...
            ax[k, j].hist(y, bins, density=True, histtype='step', cumulative=True,
                          label="Simulated {:1.0f} trials".format(trials_count), edgecolor='black', linewidth=2)

            ax[k, j].plot(x, y3, '--', linewidth=1.5, label='Poisson')
            ax[k, j].plot(x, y2, '--', linewidth=1.5, label='Minimum of uniforms')

            ax[k, j].set_title("Obstacle density: {:1.0f} per km^2 / {:1.1f} obstacles per CA".format(
                obstacle_density[c], obstacle_density[c] / 1e6 * width * distance_glide_slide), fontsize=11)
            c = c + 1
    ...
    fig.suptitle('Comparison of simulated and approximated CDF for CA of {:1.0f} m$^2$'.format(
        distance_glide_slide * width), fontsize=14)
