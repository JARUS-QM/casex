...
Example 6
---------
This example is based on the concept from the previous example (example 4), where an iso-curve for the lethal area was
generated. In that example a yellow curve showed the iso-curve for 10 m^2 lethal area and for fixed aircraft parameters.
        ...
        for f in np.array([0.3, 0.7]):
            ...
            for k in np.arange(0, width.size):
                ...
                for i, x in enumerate(x_speed):
                    Z_CA[:, i] = \
                    CA.critical_area(casex.enums.CriticalAreaModel.JARUS, aircraft, x_speed[i], y_angle, 0)[0]
                #                for i, x in enumerate(X_speed):
